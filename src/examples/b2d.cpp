...
// NOTE: example taken from blend2d.
// shows how to integrate a ny software rendering window with blend2d.
...
void MyWindowListener::draw(const ny::DrawEvent&) {
	...
	BLImage blimage;
	blimage.createFromData(image.size.x, image.size.y,
		BL_FORMAT_PRGB32, image.data, ny::byteStride(image));
	...
	// optional: schedule next frame for continous redraw
