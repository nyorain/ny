...
void displayKeyboard(bool pShow, JavaVM* lJavaVM, JNIEnv* lJNIEnv, jobject lNativeActivity) {
    ...
    jmethodID MethodGetWindow = lJNIEnv->GetMethodID(
        ClassNativeActivity, "getWindow", "()Landroid/view/Window;");
...
// Seems like ANativeActivity soft input isn't working on android.
// Internet reports the same...
bool AndroidAppContext::showKeyboard() const
{
	if(!nativeActivity_)
		return false;

	displayKeyboard(true, nativeActivity_->vm, nativeActivity_->env, nativeActivity_->clazz);
...
bool AndroidAppContext::hideKeyboard() const
{
	...
	displayKeyboard(false, nativeActivity_->vm, nativeActivity_->env, nativeActivity_->clazz);
