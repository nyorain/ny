...
//
//
// Copyright (c) 2016 Denilson das Merêcs Amorim
//
...
private:
    ...
    void construct(ValueType&& value)
    {
		...
		using Manager = typename std::conditional<requires_allocation<ValueType>::value,
			DynamicManager, StackManager>::type;

        this->vtable = vtable_for_type<T>();
		Manager::create(storage, std::forward<ValueType>(value));
    }

	struct DynamicManager
	{
		template<typename ValueType>
		static void create(storage_union& storage, ValueType&& value)
		{
        	using T = typename std::decay<ValueType>::type;
...
using anyns::any;
using anyns::bad_any_cast;
using anyns::any_cast;
