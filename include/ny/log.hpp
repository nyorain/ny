...
public:
	...
	/// Writes the given string to whatever output this object refers to.
	virtual void write(const std::string& text) const = 0;

	/// Outputs the given arguments by converting them to a string using a stringstream.
	/// Will write them to whatever output this object refers to.
	template<typename... Args>
	void output(Args&&... args)
	{
		(sstream_ << ... << std::forward<Args>(args));
		write(sstream_.str());
		sstream_.str("");
	}

	/// Operator shortcut for the output function.
	template<typename... Args>
...
public:
	Stream* stream {}; // the stream this Logger will use
	std::string logPrefix {}; // the prefix this Logger will output before every log
...
/// ```cpp
/// ny::logLogger = std::make_unique<ny::DummyStream>();
/// ```
class DummyStream : public LoggerBase {
	void write(const std::string&) const override {}
};

// The different logger objects
// Usually not used directly, prefer to use the direct printing functions (such as ny::warning).
// These functions are only useful to receive the underlaying ny::Logger object, e.g. to
// change it.
...
inline void error(Args&&... args)
	{ errorLogger()->output(std::forward<Args>(args)...); }
...
inline void warning(Args&&... args)
	{ warningLogger()->output(std::forward<Args>(args)...); }
...
inline void log(Args&&... args)
	{ logLogger()->output(std::forward<Args>(args)...); }
