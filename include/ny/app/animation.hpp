...
/*
    void addPoint(const animationPoint<O>& p){ animation<O>::addPoint(p); }
    void addPoint(double point, O value){ animation<O>::addPoint(point, value); }

    void setTime(duration t){ animation<O>::setTime(t); }

    void start(){ animation<O>::start(); }
    void stop(){ animation<O>::stop(); }
    void reset(){ animation<O>::reset(); }

    bool running() const { return animation<O>::running(); }
    duration getElapsedTime() const { return animation<O>::getElapsedTime(); }

    O& getValue(){ return animation<O>::getValue(); }
    const O& getValue() const { return animation<O>::getValue(); }
*/
};
